...
public class CustomClass {
	...
	public static void main(String[] args) {
		...
		// Show Course whose score is greater than 95
		
		courses.stream().filter(course->course.getReviewScore()>95).forEach(System.out::println);
		System.out.println(courses.stream().filter(course->course.getReviewScore()>95).collect(Collectors.toList()));
		
		//comparingByNoOfStudents
		
		System.out.println("comparingByNoOfStudents");
		...
		courses.stream().sorted(new Comparator<Course>() {
			...
			public int compare(Course o1, Course o2) {
				...
				if(o1.getNoOfStudents()-o2.getNoOfStudents()>0)
					return 1;
		...
		//comparingByNoOfStudentsAndNoOfReviews 
		
		System.out.println("comparingByNoOfStudentsAndNoOfReviews ");
		courses.stream().sorted(
				Comparator.comparingInt
				(Course::getNoOfStudents).thenComparing(Course::getReviewScore).reversed()
				).collect(Collectors.toList()).forEach(System.out::println);
		...
		courses.stream().sorted(
				Comparator.comparingInt
				(Course::getNoOfStudents).thenComparing(Course::getReviewScore).reversed()
				).collect(Collectors.mapping(Course::getName, Collectors.toList())).forEach(System.out::println);
